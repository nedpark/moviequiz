...
AI 제스처 컨트롤 (데스크탑 전용):
- 데스크탑 모드에서 퀴즈 화면 우측 하단에 웹캠 미리보기를 표시해줘.
- MediaPipe Hands 및 Drawing Utils를 사용하여 손가락 마디와 연결선을 실시간으로 화면에 그려줘.
- 손가락 개수(1~4)를 인식하고 해당하는 답안 번호를 실시간으로 하이라이트(노란색 테두리 및 그림자)해줘.
- 별도의 확인 동작 없이, 10초 제한 시간이 종료되는 시점에 마지막으로 인식된 번호를 자동으로 제출해줘.
- 매 문제가 시작되거나 종료될 때 제스처 인식 상태와 캔버스 드로잉을 즉시 초기화하여 잔상이 남지 않도록 해줘.
- 퀴즈 화면 이외의 화면(결과, 영상 재생 등)에서는 카메라 사용을 중지해줘.
