...
### 스테이지 진행 조건
1. 각 스테이지마다 **무한 문제 풀에서 무작위로 문제 선택**
2. **5문제를 연속으로 맞춰야만(5/5) 다음 스테이지 진행**
3. 한 문제라도 틀리면 **연속 정답 카운트가 초기화되고 새로운 5문제 세트 시작**
4. **최대 3번까지 실수 가능** - 각 실수는 상단 우측 원으로 표시
5. **4번째 실수 시 해당 스테이지에서 탈락** (Game Over)
...
- **상단 스테이터스 바**:
  ...
  - 중앙: 5개 원 표시 (정답 시 녹색 ● 으로 하나씩 채워짐 → 5개 완성 시 클리어)
